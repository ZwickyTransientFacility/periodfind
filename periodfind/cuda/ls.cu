...
#pragma unroll
    for (size_t curve_idx = 0; curve_idx < num_batched_curves; curve_idx++) {
        ...
        float cos_tau, sin_tau;
        __sincosf(0.5 * atan2f(2.0 * cos_sin, cos_cos - sin_sin), &sin_tau,
                  &cos_tau);
...
void LombScargle::CalcLSBatched(const std::vector<float*>& times,
    ...
    const size_t buffer_bytes =
        num_batched_curves * buffer_length * sizeof(float);
    ...
    gpuErrchk(
        cudaMalloc(&dev_lengths_buffer, num_batched_curves * sizeof(size_t)));
...
#pragma unroll
    for (size_t batch_idx = 0; batch_idx < lengths.size();
         batch_idx += num_batched_curves * num_streams) {
        for (size_t stream_idx = 0; stream_idx < num_streams; stream_idx++) {
            const size_t stream_batch_idx =
                batch_idx + stream_idx * num_batched_curves;
            if (stream_batch_idx >= lengths.size()) {
                break;
            }

            size_t curve_bytes = 0;
            const size_t num_curves_to_process =
                stream_batch_idx + num_batched_curves < lengths.size()
                    ? num_batched_curves
                    : lengths.size() - stream_batch_idx;
            const size_t actual_per_out_size =
                num_curves_to_process * per_points * sizeof(float);

            for (size_t i = 0; i < num_curves_to_process
                               && stream_batch_idx + i < lengths.size();
                 i++) {
            ...
            gpuErrchk(cudaMemcpyAsync(
                ...
                curve_bytes, cudaMemcpyHostToDevice, streams[stream_idx]));
            gpuErrchk(
                cudaMemcpyAsync(dev_lengths_buffer, &lengths[stream_batch_idx],
                                num_curves_to_process * sizeof(size_t),
                                cudaMemcpyHostToDevice, streams[stream_idx]));

            LombScargleKernelBatched<<<grid_dim, block_dim, 0,
                                       streams[stream_idx]>>>(
                dev_times_buffer, dev_mags_buffer, dev_lengths_buffer,
                dev_periods, dev_period_dts, num_periods, num_p_dts,
                num_curves_to_process, dev_per);

            gpuErrchk(cudaMemcpyAsync(&per_out[stream_batch_idx * per_points],
                                      dev_per, actual_per_out_size,
