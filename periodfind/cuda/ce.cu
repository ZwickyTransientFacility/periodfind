...
ConditionalEntropy::ConditionalEntropy(size_t n_phase, size_t n_mag,
                                       size_t p_overlap, size_t m_overlap) {
 ...
 */
__global__ void FoldBinKernel(const float *times, const float *mags,
                              const size_t length, const float *periods,
                              const float *period_dts,
                              const ConditionalEntropy h_params, float *hists) {
  ...
  for (size_t i = threadIdx.x; i < h_params.NumBins(); i += blockDim.x) {
    block_hist[i] = static_cast<float>(sh_hist[i]) / static_cast<float>(length);
  }
}
...
float *ConditionalEntropy::DeviceFoldAndBin(
    const float *times, const float *mags, const size_t length,
    const float *periods, const float *period_dts, const size_t num_periods,
...
float *ConditionalEntropy::FoldAndBin(const float *times, const float *mags,
                                      const size_t length, const float *periods,
                                      const float *period_dts,
  ...
  float *dev_hists = DeviceFoldAndBin(dev_times, dev_mags, length, dev_periods,
...
float *ConditionalEntropy::DeviceCalcCEFromHists(const float *hists,
  ...
  // NOTE: A ConditionalEntropy object is small enough that we can pass it in
  //       the registers by dereferencing it.
  ConditionalEntropyKernel<<<n_b, n_t, n_t * sizeof(float)>>>(hists, num_hists,
                                                              *this, dev_ces);
...
float *ConditionalEntropy::CalcCEVals(const float *times, const float *mags,
                                      const size_t length, const float *periods,
                                      const float *period_dts,
                                      const size_t num_periods,
                                      const size_t num_p_dts) const {
  ...
  // Copy CEs to host
  float *ces = (float *)malloc(num_hists * sizeof(float));
  gpuErrchk(cudaMemcpy(ces, dev_ces, num_hists * sizeof(float), cudaMemcpyDeviceToHost));
...
float *ConditionalEntropy::CalcCEValsBatched(const std::vector<float *> &times,
  ...
  float *ce_host = (float *)malloc(ce_size_total);
  ...
  const size_t num_threads_fb = 512;
  ...
  const size_t num_threads_ce = 512;
  ...
  for (size_t i = 0; i < lengths.size(); i++) {
    ...
    gpuErrchk(cudaMemcpy(dev_times_buffer, times[i], curve_bytes, cudaMemcpyHostToDevice));
    ...
    ConditionalEntropyKernel<<<num_blocks_ce, num_threads_ce,
                               shared_bytes_ce>>>(dev_hists, num_hists, *this,
                                                  dev_ces);

    // Copy CE data back to host
    gpuErrchk(cudaMemcpy(&ce_host[i * num_hists], dev_ces, ce_out_size,
               cudaMemcpyDeviceToHost));
  }
