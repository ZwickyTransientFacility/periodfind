									  ...
									  const size_t num_p_dts) const
{
	...
	gpuErrchk(cudaMemcpy(dev_periods, period_dts, num_p_dts * sizeof(float), cudaMemcpyHostToDevice));

	float *dev_hists = DeviceFoldAndBin(dev_times, dev_mags, length, dev_periods, dev_period_dts, num_periods, num_p_dts);
...
float *ConditionalEntropy::DeviceCalcCEFromHists(const float *hists, const size_t num_hists) const
{
...
float *ConditionalEntropy::CalcCEFromHists(const float *hists, const size_t num_hists) const
{
											 ...
											 const size_t                num_p_dts) const
{
	...
	CalcCEValsBatched(times, mags, lengths, periods, period_dts, num_periods, num_p_dts, ce_out);
