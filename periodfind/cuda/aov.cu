						  ...
						  float *__restrict__ aovs)
{
	...
	aovs[thread_id] = (static_cast<float>(length - aov.NumPhaseBins()) / static_cast<float>(aov.NumPhaseBins() - 1)) * (s1 / s2);
}
						 ...
						 const size_t num_p_dts) const
{
	...
	AOVData *dev_hists = DeviceFoldAndBin(dev_times, dev_mags, length, dev_periods, dev_period_dts, num_periods, num_p_dts);
							   ...
							   const size_t                num_p_dts) const
{
	...
	CalcAOVValsBatched(times, mags, lengths, periods, period_dts, num_periods, num_p_dts, aov_out);
